...
return function(_, bufnr)
  ...
  nmap("gr", vim.lsp.buf.references, "LSP(builtin): Goto references", bufnr)
  -- 符号列表
  nmap("<leader>ls", "<cmd>lua require('telescope.builtin').lsp_document_symbols()<cr>",
    "LSP(Telescope): list document symbols", bufnr)
  nmap("<leader>lS",
    "<cmd>lua require('telescope.builtin').lsp_workspace_symbols()<cr>",
    "LSP(Telescope): list workspace symbols", bufnr)
  -- 函数签名 打开函数签名文档并直接调转到文档窗口上
  nmap("K", function() vim.lsp.buf.hover() vim.lsp.buf.hover() end,
    "LSP(builtin): Hover documentation", bufnr)
  -- nmap("<M-k>", vim.lsp.buf.signature_help,
  ...
  nmap("<leader>ld", "<cmd>lua require('telescope.builtin').diagnostics({ bufnr = 0 })<cr>",
    "LSP(Telescope): List current buffer diagnostics", bufnr)
  nmap("<leader>lD", "<cmd>lua require('telescope.builtin').diagnostics()<cr>",
